...
# Bandera de Error de Dominio

_IK_FAILURE_DETECTED = False

def has_ik_failed():
    return _IK_FAILURE_DETECTED

def reset_ik_failure_flag():
    global _IK_FAILURE_DETECTED
    _IK_FAILURE_DETECTED = False

def checkdomain(D):
    if D > 1 or D < -1:
        print("____OUT OF DOMAIN____")
        _IK_FAILURE_DETECTED = True
        if D > 1: 
            D = 0.99
            return D
        elif D < -1:
